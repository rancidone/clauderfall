...
New tool in `server.py`, no required parameters. Calls `design.list()` and returns the result directly. Sits alongside `design_read`, `design_write`, and `design_accept`.
